...
class BookReviewAPITestCase(APITestCase):
    def setUp(self):
        self.user = CustomUser.objects.create(username="abbos", first_name="Abbos")
        self.user.set_password("abbos1234")
        self.user.save()
        self.client.login(username="abbos", password="abbos1234")

    def test_book_review_detail(self):
        book = Book.objects.create(title="Book1", description="Description1", isbn="15324")
        ...
        self.assertEqual(response.data['book']['isbn'], '15324')
        self.assertEqual(response.data['user']['id'], self.user.id)
        self.assertEqual(response.data['user']['first_name'], "Abbos")
        self.assertEqual(response.data['user']['username'], "abbos")
